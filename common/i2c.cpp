...
I2C::I2C(QObject *parent): QObject(parent)
{
    ...
    qDebug() << "[I2C] constructor addr";
}

/*
 **	@brief		:	Main setup function
 **	#bus_name	:	address of device on I2C buss
 */
void I2C::setAddress(int address)
{
    devAddr = address;
    qDebug() << "[I2C] Address =" << devAddr ;
}
 ...
 **	#bus_name	:	address of device on I2C buss
 */
void I2C::setBuss(QString bus)
{
    //open
    //i2c_init_device

    const char* myChar = bus.toStdString().c_str();
    int fd = i2c_open(myChar);
    if (fd < 0)
    {
    ...
    device.bus = fd;
    qDebug() << "[I2C] Buss = " << bus << " FD =" << device.bus;

    i2c_init_device( &device);
    device.addr = devAddr;

//    qDebug() << "[I2C] Params - bus =" << device.bus << " address =" << device.addr;
 ...
 */
 int I2C::readAddress()
 {
//     qDebug() << "[I2C] BusR = " << buss;
//     qDebug() << "[I2C] Address =" << addr;
    return device.addr;
 }

 void I2C::setSensor(int sensor)
 {
     qDebug() << "Set Sensor =" << sensor;
 }

 int I2C::readSensor()
 {
     memset(buf, 17, 16);
     int rtn = i2c_ioctl_read(&device, 2 , (__u8 *) &buf[0], 1);
     if(rtn <0)
     {
         qDebug() << "[I2C] i2c_read - ERROR! ";
         return -1;
     }
     rtn = i2c_ioctl_read(&device, 3 , (__u8 *) &buf[1], 1);
     if(rtn <0)
     {
         qDebug() << "[I2C] i2c_read - ERROR! ";
         return -1;
     }
     int calc = (buf[0] * 256) + buf[1];
//    qDebug() << "[I2C] Read Sensor Value =" << calc;

     if (calc != sensorVal)
     {
         emit sensorChanged(calc);
         sensorVal = calc;
         MyGlobal.insert("luxSensor", calc);
     }
     return calc;
}
 ...
 */
int I2C::i2c_open(const char *bus_name)
{
    int fd;
 ...
 */
void I2C::i2c_init_device(I2CDevice *device)
{
 ...
 */
char * I2C::i2c_get_device_desc(const I2CDevice *device, char *buf, size_t size)
{
    memset(buf, 0, size);
 ...
 **
 */
ssize_t I2C::i2c_ioctl_read(const I2CDevice *device, unsigned int iaddr, __u8 *buf, size_t len)
{
...
ssize_t I2C::i2c_ioctl_write(const I2CDevice *device, unsigned int iaddr, const unsigned char *buf, size_t len)
{
    ssize_t remain = len;
    size_t size = 0, cnt = 0;
 ...
 */
ssize_t I2C::i2c_read(const I2CDevice *device, unsigned int iaddr, __u8 *buf, size_t len)
{
    ssize_t cnt;
    unsigned char addr[INT_ADDR_MAX_BYTES];
    unsigned char delay = GET_I2C_DELAY(device->delay);
    ...
    qDebug() << "[I2C] IADDR =" << device->iaddr_bytes << " 10Bit " << device->tenbit;
 ...
 */
ssize_t I2C::i2c_write(const I2CDevice *device, unsigned int iaddr, const unsigned char *buf, size_t len)
{
    ssize_t remain = len;
    ssize_t ret;
    size_t cnt = 0, size = 0;
 ...
 */
void I2C::i2c_iaddr_convert(unsigned int iaddr, unsigned int len, unsigned char *addr)
{
    union
    {
        unsigned int iaddr;
 ...
 */
int I2C::i2c_select(int bus, unsigned long dev_addr, unsigned long tenbit)
{
 ...
 */
void I2C::i2c_delay(unsigned char msec)
{
    usleep(msec *1e3);
}
